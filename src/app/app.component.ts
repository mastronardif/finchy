...
export class AppComponent {
      ...
      if (this_row["AskerPhoto"]) {
        question.setPhoto(
          this._domSanitizer.bypassSecurityTrustResourceUrl(
            "data:image/jpg;base64," + this_row["AskerPhoto"]
          )
        );
      }
  ...
  fetchAndLoadAnswers(questionIndex, questionId) {
    ...
    this._finchyService
      .executeQuery(domain, query, params)
      .subscribe((response) => {
        const result = response.queryResult;
        while (result.moveToNextRow()) {
          const this_row = {};
          ...
          if (this_row["AnswererPhoto"]) {
            answer.setPhoto(
              this._domSanitizer.bypassSecurityTrustResourceUrl(
                "data:image/jpg;base64," + this_row["AnswererPhoto"]
              )
            );
          }
  ...
  submitAnswer(questionIndex, questionId) {
    ...
    this._finchyService
      ...
      .subscribe((response) => {
        ...
        this.openConnectionForAnswerTransaction(
          this.currentAnswer,
          questionIndex,
          questionId,
          userId
  ...
  openConnectionForAnswerTransaction(
    answer,
    questionIndex,
    questionId,
    userId
  ...
  sendAnswer(
    ...
    this._finchyService
      .executeQuery(domain, queryAnswer, paramsAnswer)
      .subscribe(
